...
void Analyze::check_clause(const std::vector<std::string>& tab_names,
    ...
    for (auto& cond : conds) {
        ...
        if (lhs_type != rhs_type) {
            if (lhs_type == ColType::TYPE_INT &&
                rhs_type == ColType::TYPE_FLOAT) {
            } else if (lhs_type == ColType::TYPE_FLOAT &&
...
Value Analyze::convert_sv_value(const std::shared_ptr<ast::Value>& sv_val) {
    ...
    } else if (auto dateTime_lit =
                   std::dynamic_pointer_cast<ast::DateTimeLit>(sv_val)) {
        if (!is_valid_date(dateTime_lit->val)) {
            throw DateTimeAbsurdError("", dateTime_lit->val);
        }
