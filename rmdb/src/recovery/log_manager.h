...
public:
    ...
    UpdateLogRecord(txn_id_t txn_id, const RmRecord& old_rec, const RmRecord& new_rec, const Rid& rid, const std::string& table_name)
        : UpdateLogRecord() {
        log_tid_ = txn_id;
        old_record_ = old_rec;
        new_record_ = new_rec;
        ...
        memcpy(table_name_, table_name.c_str(), table_name_size_);
        log_tot_len_ += sizeof(int);
        log_tot_len_ += old_record_.size;
        log_tot_len_ += sizeof(int);
        log_tot_len_ += new_record_.size;
        log_tot_len_ += sizeof(Rid);
        log_tot_len_ += sizeof(size_t) + table_name_size_;
    }

    // Serialize the update log record to destination buffer
    void serialize(char* dest) const override {
        LogRecord::serialize(dest);
        int offset = OFFSET_LOG_DATA;
    ...
    // Deserialize the update log record from source buffer
    void deserialize(const char* src) override {
        LogRecord::deserialize(src);
        int offset = OFFSET_LOG_DATA;
        old_record_.size = *reinterpret_cast<const int*>(src + offset);
        offset += sizeof(int);
        old_record_.data = new char[old_record_.size];
        memcpy(old_record_.data, src + offset, old_record_.size);
        offset += old_record_.size;
        new_record_.size = *reinterpret_cast<const int*>(src + offset);
        offset += sizeof(int);
        new_record_.data = new char[new_record_.size];
        memcpy(new_record_.data, src + offset, new_record_.size);
        offset += new_record_.size;
        rid_ = *reinterpret_cast<const Rid*>(src + offset);
        offset += sizeof(Rid);
        table_name_size_ = *reinterpret_cast<const size_t*>(src + offset);
    ...
    // Format print the update log record
    void format_print() override {
...
private:
    RmRecord old_record_;  // Old record data
    RmRecord new_record_;  // New record data
    Rid rid_;              // Record ID of the updated record
    char* table_name_;     // Table name where the update occurred
    size_t table_name_size_;  // Size of table name
};
